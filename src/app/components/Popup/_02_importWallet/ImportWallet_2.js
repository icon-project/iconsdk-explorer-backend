...
import { validateKSFile, validateIconsdkFile } from 'utils';
    ...
    files.forEach(file => {
      ...
      reader.onload = () => {
        ...
        new Promise(resolve => {
          ...
          } else if (validateIconsdkFile(ks)) {
            this.setState({
              file: ks,
              fileTitle: file.name,
              isDropped: true,
  ...
  handleSubmit = () => {
    ...
    if (file) {
      this.setState({ loading: true }, () => {
        this.worker.postMessage({ file: file, pw: pw, type: fileType === 'ks' ? 'importWallet_2_ks' : 'importWallet_2_iconsdk' });
      })
    } else {
      this.validateForm();
    }
