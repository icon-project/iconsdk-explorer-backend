...
function generateIconsdkObject(key, coinType, walletName, v3) {
  const value = {
    'type': coinType,
    'name': walletName,
    'priv': v3,
...
function downloadFile(address, data, FileSaver) {
  ...
  if (address) {
    ...
    filename = "iconsdk_" + new Date().toISOString();
  }
...
function validateIconsdkFile(iconsdkFile) {
  if (Array.isArray(iconsdkFile)) {
    for (let i = 0; i < iconsdkFile.length; i++) {
      const walletData = iconsdkFile[i][Object.keys(iconsdkFile[i])[0]];
