...
import { coinRound as COIN_ROUND } from 'constants/index';
...
function convertNumberToText(num, unit, isCoin) {
  const ROUND = isCoin ? COIN_ROUND : COIN_ROUND;
...
function trimLeftZero(str) {
  ...
  // if string is a single 0 replace character
...
function calcTokenBalance(balance, defaultDecimals, decimals) {
  if (typeof balance === 'object') {
    return balance.div(Math.pow(10, defaultDecimals - decimals)).toNumber();
  }
  return balance / Math.pow(10, defaultDecimals - decimals);
}
