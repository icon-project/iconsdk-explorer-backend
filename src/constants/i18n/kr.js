export default {
  ...
  myPageInfo1:
    "· 잠금 설정 시 ICONSDK를 실행할 때마다 잠금 번호를 확인합니다. 타인에게 내 지갑 정보가 노출되는 것을 방지할 수 있습니다.\n· 잠금 번호를 잊은 경우, 지갑 비밀번호 확인 후 잠금 번호를 재설정 할 수 있습니다.",
  myPageInfo2:
    "· 잠금 설정 시 ICONSDK를 실행할 때마다 잠금 번호를 확인합니다. 다른 사람에게 내 자산을 예기치 않게 노출하는 것을 방지할 수 있습니다.",
  myPageInfo3:
    "· 지갑 묶어서 내보내기는 ICONSDK 지갑 파일들을 모두 묶어서 하나의 파일로 백업하는 기능입니다.\n · 묶음 지갑의 Keystore 파일을 불러오면 여러 개의 지갑을 한번에 불러올 수 있습니다.",
  ...
  disclaimerPage: {
    header: "ICONSDK 이용 면책 조항",
    title:
      "ICX 공식 wallet “ICONSDK”의 이용자는 “ICONSDK”를 다운로드하여 이용함과 동시에 아래 면책 조항에 대한 구속력이 발생함을 알려드립니다.",
    desc1:
      "1. ICONSDK 이용자가 ICONSDK 이용 과정에서 생성한 개인 키는 사용자의 로컬 PC에 저장되며, ICON Foundation에서는 ICONSDK이용자의 개인 키를 별도로 보관ㆍ관리하고 있지 않습니다. ICONSDK 이용자는 본인이 생성한 개인키에 대한 보관 및 관리의 책임이 있으며, 바이러스, 해킹, 분실 등 각종 원인으로ICONSDK 이용자가 개인키를 이용할 수 없게 된 경우 ICON Foundation은 이에 대한 일체의 책임을 부담하지 않습니다.",
    desc2:
      "2. ICON Foundation은 ICONSDK의 무결성, 안정성, 정확성, 보안성 등에 대하여 보증하지 않으며, 현재 버전의 ICONSDK를 ‘그 자체로’ 제공합니다.  ICON Foundation은 더 안정적이고 편리한 서비스를 제공하기 위하여 ICONSDK에 대하여 지속적으로 테스트와 업데이트를 시행하고 있으나, 이러한 노력에도 불구하고 예기치 못한 문제가 발생할 가능성은 항상 존재합니다. ICON Foundation은 오작동, 해킹, 분실, 바이러스 감염 등을 포함한 일체의 예기치 못한 문제에 대하여 책임을 부담하지 않습니다.",
    desc3:
    ...
    desc4:
      "4. ICON Foundation은 ICONSDK 이용자가 ICONSDK를 매개로 하여 제3자와 거래 등을 한 경우 이와 관련하여 책임을 부담하지 않습니다.",
    desc5:
      "5. ICON Foundation은 ICONSDK 이용과 관련하여 불법행위, 계약, 보증, 무과실 책임, 부주의로 인한 어떠한 형태의 책임을 부담하는 경우라 하더라도, 간접적, 결과적, 특수적, 징벌적 또는 이와 유사한 손해에 대해서는 일체의 책임을 부담하지 않습니다.",
    desc6:
      "6. ICONSDK는 한국어와 영어를 기본 언어로 지원합니다. 기본으로 제공하는 언어 외에 다른 언어로 번역 시 이로 인해 의미가 잘못 전달되어 발생한 문제에 대해서는 ICON Foundation에게 책임이 없습니다.",
    copyright:
      "※ ICONSDK와 관련된 모든 저작권은 ICON Foundation에 있습니다 (Copyright © 2020 ICON Foundation).",
  },
  ...
  connectLedger: {
    ...
    info: "ICONSDK에서 Ledger Wallet 이용하는 방법",
    walletAddress: "지갑 주소",
    connectWallet: "연결된 Ledger Wallet",
    manualFileName: "Guide_to_use_Ledger_Wallet_on_ICONSDK_Ledger_kr",
  },
